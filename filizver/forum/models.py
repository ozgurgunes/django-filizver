...
from filizver.forum.util import unescape
...
def get_file_path(instance, filename, to='forum/avatar'):
    """
...
class Category(models.Model):
    ...
    def get_absolute_url(self):
        return reverse('forum:category', kwargs={'pk': self.id})
...
class Forum(models.Model):
    ...
    moderators = models.ManyToManyField(User, blank=True, null=True, verbose_name=_('Moderators'), related_name='forum_moderator')
    updated = models.DateTimeField(_('Updated'), blank=True, null=True)
    post_count = models.IntegerField(_('Post count'), blank=True, default=0)
    topic_count = models.IntegerField(_('Topic count'), blank=True, default=0)
    hidden = models.BooleanField(_('Hidden'), blank=False, null=False, default=False)
    readed_by = models.ManyToManyField(User, through='ForumReadTracker', related_name='forums_read')
    ...
    def get_absolute_url(self):
        return reverse('forum:forum', kwargs={'pk': self.id})
...
class Post(RenderableItem, EntryBase):
    ...
    def get_absolute_url(self):
        return reverse('forum:post', kwargs={'pk': self.id})
...
class Profile(PybbProfile):
    ...
    def get_absolute_url(self):
        return reverse('forum:user', kwargs={'username': self.user.username})
...
class TopicReadTracker(models.Model):
    ...
    user = models.ForeignKey(User, blank=False, null=False, related_name='read_topics')
...
class ForumReadTracker(models.Model):
    ...
    user = models.ForeignKey(User, blank=False, null=False, related_name='read_forums')
    forum = models.ForeignKey(Forum, blank=True, null=True)
    time_stamp = models.DateTimeField(auto_now=True)



from filizver.forum import signals
