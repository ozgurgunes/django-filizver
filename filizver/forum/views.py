...
try:
    ...
    try:
        ...
        raise ImportError('If you using django version < 1.3 you should install django-cbv for forum')
...
class TopicView(generic.ListView):
    ...
    def get_queryset(self):
        ...
        if self.topic.on_moderation and\
           not forum_topic_moderated_by(self.topic, self.request.user) and\
        ...
        if not forum_topic_moderated_by(self.topic, self.request.user):
            if self.request.user.is_authenticated():
                qs = qs.filter(Q(user=self.request.user)|Q(on_moderation=False))
            else:
                qs = qs.filter(on_moderation=False)
...
class AddPostView(PostEditMixin, generic.CreateView):
    ...
    def dispatch(self, request, *args, **kwargs):
        ...
        if not self.user.has_perm('forum.add_post'):
            raise PermissionDenied
...
class EditPostView(PostEditMixin, generic.UpdateView):
    ...
    def get_object(self, queryset=None):
        ...
        if not forum_editable_by(post, self.request.user):
            raise PermissionDenied
        return post
...
class PostView(generic.RedirectView):
    def get_redirect_url(self, **kwargs):
        ...
        if defaults.PYBB_PREMODERATION and\
            ...
            (not forum_topic_moderated_by(post.topic, self.request.user)) and\
            (not post.user==self.request.user):
            raise PermissionDenied
        count = post.topic.posts.filter(created__lt=post.created).count() + 1
        page = math.ceil(count / float(defaults.PYBB_TOPIC_PAGE_SIZE))
        return '%s?page=%d#post-%d' % (reverse('forum:topic', args=[post.topic.id]), page, post.id)


class ModeratePost(generic.RedirectView):
    def get_redirect_url(self, **kwargs):
        post = get_object_or_404(Post, pk=self.kwargs['pk'])
        if not forum_topic_moderated_by(post.topic, self.request.user):
            raise PermissionDenied
...
class ProfileEditView(generic.UpdateView):
    ...
    def get_success_url(self):
        return reverse('forum:edit_profile')
...
class TopicActionBaseView(generic.View):
    ...
    def get_topic(self):
        ...
        if not forum_topic_moderated_by(topic, self.request.user):
            raise PermissionDenied
        return topic
...
@login_required
@permission_required('forum.block_users')
