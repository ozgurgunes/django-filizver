...
from filizver.forum.models import *
...
class FeaturesTest(TransactionTestCase, SharedTestModule):
    ...
    def test_profile_edit(self):
        ...
        response = self.client.post(reverse('forum:edit_profile'), data=values, follow=True)
        self.assertEqual(response.status_code, 200)
    ...
    def test_pagination_and_topic_addition(self):
        for i in range(0, defaults.PYBB_FORUM_PAGE_SIZE + 3):
            ...
            topic.save()
        url = reverse('forum:forum', args=[self.forum.id])
        response = self.client.get(url)
    ...
    def test_topic_addition(self):
        self.login_client()
        add_topic_url = reverse('forum:add_topic', kwargs={'forum_id': self.forum.id})
    ...
    def test_read_tracking(self):
        ...
        # Forum status
        tree = html.fromstring(client.get(reverse('forum:index')).content)
        ...
        # Post message
        add_post_url = reverse('forum:add_post', kwargs={'topic_id': topic.id})
        ...
        post = Post(topic=topic, user=self.user, body='one')
        post.save()
        client.get(reverse('forum:mark_all_as_read'))
        tree = html.fromstring(client.get(reverse('forum:index')).content)
        self.assertFalse(tree.xpath('//a[@href="%s"]/parent::td[contains(@class,"unread")]' % topic.forum.get_absolute_url()))
        # Empty forum - readed
    ...
    def test_read_tracking_multi_user(self):
        ...
        add_post_url = reverse('forum:add_post', kwargs={'topic_id': topic_1.id})
    ...
    def test_hidden(self):
        ...
        self.assertFalse(category.id in [c.id for c in client.get(reverse('forum:index')).context['categories']])
        ...
        self.assertNotContains(client.get(reverse('forum:index')), forum_hidden.get_absolute_url())
        self.assertNotContains(client.get(reverse('forum:feed_topics')), topic_hidden.get_absolute_url())
        self.assertNotContains(client.get(reverse('forum:feed_topics')), topic_in_hidden.get_absolute_url())

        self.assertNotContains(client.get(reverse('forum:feed_posts')), post_hidden.get_absolute_url())
        self.assertNotContains(client.get(reverse('forum:feed_posts')), post_in_hidden.get_absolute_url())
        ...
        self.user.save()
        self.assertTrue(category.id in [c.id for c in client.get(reverse('forum:index')).context['categories']])
        self.assertContains(client.get(reverse('forum:index')), forum_hidden.get_absolute_url())
    ...
    def test_inactive(self):
        self.login_client()
        url = reverse('forum:add_post', kwargs={'topic_id': self.topic.id})
        response = self.client.get(url)
        values = self.get_form_values(response)
    ...
    def test_csrf(self):
        ...
        client.login(username='zeus', password='zeus')
        post_url = reverse('forum:add_post', kwargs={'topic_id': self.topic.id})
        ...
        values = self.get_form_values(response)
        response = client.post(reverse('forum:add_post', kwargs={'topic_id': self.topic.id}), values, follow=True)
        self.assertEqual(response.status_code, 200)
    ...
    def test_user_blocking(self):
        ...
        self.user.save()
        self.login_client()
        self.assertEqual(self.client.get(reverse('forum:block_user', args=[user.username]), follow=True).status_code, 200)
        user = User.objects.get(username=user.username)
        self.assertFalse(user.is_active)

    def test_ajax_preview(self):
        self.login_client()
        response = self.client.post(reverse('forum:post_ajax_preview'), data={'data': '[b]test bbcode ajax preview[b]'})
    ...
    def test_quote(self):
        self.login_client()
        response = self.client.get(reverse('forum:add_post', kwargs={'topic_id': self.topic.id}), data={'quote_id': self.post.id, 'body': 'test tracking'}, follow=True)
        self.assertEqual(response.status_code, 200)
        self.assertContains(response, self.post.body)

    def test_edit_post(self):
        self.login_client()
        edit_post_url = reverse('forum:edit_post', kwargs={'pk': self.post.id})
    ...
    def test_admin_post_add(self):
        ...
        self.user.save()
        self.login_client()
        response = self.client.post(reverse('forum:add_post', kwargs={'topic_id': self.topic.id}), data={'quote_id': self.post.id, 'body': 'test admin post', 'user': 'zeus'}, follow=True)
        self.assertEqual(response.status_code, 200)
    ...
    def test_stick(self):
        ...
        self.user.save()
        self.login_client()
        self.assertEqual(self.client.get(reverse('forum:stick_topic', kwargs={'pk': self.topic.id}), follow=True).status_code, 200)
        self.assertEqual(self.client.get(reverse('forum:unstick_topic', kwargs={'pk': self.topic.id}), follow=True).status_code, 200)
    ...
    def test_delete_view(self):
        ...
        self.user.save()
        self.login_client()
        response = self.client.post(reverse('forum:delete_post', args=[post.id]), follow=True)
        self.assertEqual(response.status_code, 200)
        ...
        # Delete topic
        response = self.client.post(reverse('forum:delete_post', args=[self.post.id]), follow=True)
        self.assertEqual(response.status_code, 200)
    ...
    def test_open_close(self):
        ...
        self.user.save()
        self.login_client()
        add_post_url = reverse('forum:add_post', args=[self.topic.id])
        response = self.client.get(add_post_url)
        values = self.get_form_values(response)
        values['body'] = 'test closed'
        response = self.client.get(reverse('forum:close_topic', args=[self.topic.id]), follow=True)
        self.assertEqual(response.status_code, 200)
        response = self.client.post(add_post_url, values, follow=True)
        self.assertEqual(response.status_code, 403)
        response = self.client.get(reverse('forum:open_topic', args=[self.topic.id]), follow=True)
        self.assertEqual(response.status_code, 200)
        response = self.client.post(add_post_url, values, follow=True)
        self.assertEqual(response.status_code, 200)
    ...
    def test_subscription(self):
        ...
        response = client.get(reverse('forum:add_subscription', args=[self.topic.id]), follow=True)
        self.assertEqual(response.status_code, 200)
        ...
        response = client.get(reverse('forum:delete_subscription', args=[self.topic.id]), follow=True)
        self.assertEqual(response.status_code, 200)
    ...
    def test_user_view(self):
        resp = self.client.get(reverse('forum:user', kwargs={'username': self.user.username}))
...
class AnonymousTest(TransactionTestCase, SharedTestModule):
    ...
    def setUp(self):
        ...
        add_post_permission = Permission.objects.get_by_natural_key('add_post', 'forum', 'post')
...
class PreModerationTest(TransactionTestCase, SharedTestModule):
    ...
    def test_premoderation(self):
        ...
        add_post_url = reverse('forum:add_post', kwargs={'topic_id': self.topic.id})
        response = self.client.get(add_post_url)
        values = self.get_form_values(response)
        ...
        post = Post.objects.get(body='test premoderation')
        response = admin_client.get(reverse('forum:moderate_post', kwargs={'pk': post.id}), follow=True)
        self.assertEqual(response.status_code, 200)
        ...
        post.save()
        client.login(username='zeus', password='zeus')
        response = client.get(reverse('forum:moderate_post', kwargs={'pk': post.id}), follow=True)
        self.assertEqual(response.status_code, 403)
        ...
        self.assertEqual(response.status_code, 403)
        response = admin_client.get(reverse('forum:moderate_post',
...
class AttachmentTest(TransactionTestCase, SharedTestModule):
    def setUp(self):
        ...
        self.ORIG_PYBB_PREMODERATION = defaults.PYBB_PREMODERATION
        defaults.PYBB_PREMODERATION = False
        self.file = open(os.path.join(os.path.dirname(__file__), 'static', 'forum', 'img','attachment.png'))
        self.create_user()
        self.create_initial()
...
class PollTest(TransactionTestCase, SharedTestModule):
    ...
    def test_poll_edit(self):
        edit_topic_url = reverse('forum:edit_post', kwargs={'pk': self.post.id})
        self.login_client()
    ...
    def test_poll_voting(self):
        ...
        vote_url = reverse('forum:topic_poll_vote', kwargs={'pk': self.topic.id})
